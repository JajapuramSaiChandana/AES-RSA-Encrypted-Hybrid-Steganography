...
def encode_txt_data():
    ...
    cover=input("\nPlease enter the stego file name(with extension) to encode the message:- ")
    file1= open(cover,"r")
    for line in file1: 
    ...
    if(l<=int(bt/6)):
        ...
        k = str.encode(k)
        padding_length = 16 - (len(k) % 16)
        # Apply PKCS7 padding
        k += bytes([padding_length]) * padding_length
        text2 = af.encrypt(text1, k)

        # print(text2)
        
        pvk = rsa.generate_private_key(
        ...
        k= pbk.encrypt(k, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))
...
def decode_txt_data():
    ...
    if an==0:
        ...
        pvk_bytes = lst[0][-1]
        pvk = serialization.load_pem_private_key(pvk_bytes, password=None)
        k= pvk.decrypt(lst[0][2], padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))


        # final1 = af.decrypt(lst[0][-1], final)
        
        final1 = af.decrypt(k, final)
        print("\nMessage after decoding from the stego file:- ",final1)
            ...
            for pixel in i:
                ...
                for byte in total_bytes:
                    ...
                    if decoded_data[-5:] == "*^*^*": 
                        ...
                        # msg = af.decrypt(lst[0][-1], data)
                        msg = af.decrypt(k, data)
                        print("Secret Message in the image is :", msg)
                        return
...
def img_steg():
    while True:
        ...
        choice1 = int(input("Enter the Choice: "))   
        if choice1 == 1:
            image=cv2.imread(input("Enter the Image you need to encode :  "))
    ...
    for i in range(len(frame_bytes)):
        ...
        for byte in all_bytes:
            ...
            if decoded_data[-5:] == "*^*^*":
                ...
                dt = af.decrypt( k, dt)
                p=1
                print("The encoded message is: ", dt)
                break  
...
def encryption(plaintext, un, pd, fn):
    print("Enter the key : ")
    key=input()
    db.vid(un, pd, fn, key)
...
def decryption(ciphertext, un, pd, fn):
    print("Enter the key : ")
    key=input()
    
    ans, lst = db.getvid(un, pd, int(fn), key)
    if ans==1:
    ...
    if ans==2:
        ...
        return
    if ans==3:
        print("The frame number you have entered is INCORRECT!")
        return
    if ans==4:
...
def embed(frame, un, pd, fn):
    data=input("\nEnter the data to be Encoded in Video :") 
    data=encryption(data, un, pd, fn)
        ...
        for pixel in i:
            ...
            for byte in total_bytes:
                ...
                if decoded_data[-5:] == "*^*^*": 
                    ...
                    final_decoded_msg = decryption(final_decoded_msg, un, pd, int(fn))
...
def encode_vid_data():
    ...
    vid = input("Enter video file name (with extension) to encode the data: ")
    un = input("Enter username: ")
    pd = input("Enter password: ")

    cap=cv2.VideoCapture("/Users/chandana/Desktop/hiii/"+vid)
    vidcap = cv2.VideoCapture("/Users/chandana/Desktop/hiii/"+vid)    
    ...
    while(vidcap.isOpened()):
        ...
        if ret == False:
            break
        if frame_number == n:    
            change_frame_with = embed(frame, un, pd, n)
...
def decode_vid_data(frame_):
    # file_name = input("Enter the name of file to be decoded: ")
